...
function CommunityTakeoversPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      >
        <h1 className="text-3xl font-bold text-gradient mb-2">Community Takeovers</h1>
                ...
                {(takeovers || []).map((token, index) => {
                  ...
                  return (
                    <motion.div
                      ...
                      className="rounded-xl border border-white/10 p-4 hover:bg-white/5 hover:border-violet-500/30 transition-all"
                    >
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-3 flex-1">
                          <div className="w-10 h-10 rounded-full bg-violet-500/20 flex items-center justify-center overflow-hidden relative">
                            {getTokenIcon(token) ? (
                              <img
                              ...
                              <span className="text-violet-400 font-bold">{token.symbol[0]}</span>
                            )}
                          </div>
                          <div className="flex-1">
                            <div className="flex items-center gap-2 flex-wrap">
                              <a
                                ...
                                target="_blank"
                                rel="noreferrer"
                                className="text-foreground font-medium hover:text-violet-400 transition-colors"
                              >
                                {token.symbol}
                              </a>
                              <span className="text-muted-foreground text-xs bg-violet-500/20 px-2 py-0.5 rounded-full">{token.chain}</span>
                            </div>
                        ...
                        <div className="flex items-center gap-4">
                          <div className="text-right">
                            ...
                            <p className={`text-xs font-mono ${(token.change24h || 0) >= 0 ? 'text-accent' : 'text-red-400'}`}>
